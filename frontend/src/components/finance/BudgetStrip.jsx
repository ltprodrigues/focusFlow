...
export function BudgetStrip({ summary, status, error, onRetry, onAddExpense, onEditBudget }) {
  ...
  if (!summary.hasBudget) {
    return (
      <section className="budget-strip budget-empty-state" aria-labelledby="budget-title">
        <div><p className="budget-eyebrow">Monthly spending</p><h2 id="budget-title">{month} budget</h2><p>Set a budget to track what is left this month.</p></div>
        <button className="budget-secondary-action" type="button" onClick={onEditBudget}>Set monthly budget</button>
      </section>
    )
  }
  ...
  return (
    <section className={`budget-strip${summary.isOverBudget ? ' is-over-budget' : ''}`} aria-labelledby="budget-title">
      <div className="budget-chart-group">
        ...
        <div><p className="budget-eyebrow">Monthly spending</p><h2 id="budget-title">{month} budget</h2><button className="budget-edit" type="button" onClick={onEditBudget}>Edit budget</button></div>
      </div>
      <dl className="budget-totals">
        <div><dt>Budget</dt><dd>{formatCad(summary.budgetAmount)}</dd></div>
        <div><dt>Spent</dt><dd>{formatCad(summary.totalSpent)}</dd></div>
        <div className="budget-remaining"><dt>{summary.isOverBudget ? 'Over' : 'Remaining'}</dt><dd>{remaining}</dd></div>
      </dl>
      <div className="budget-categories">
        {summary.totalSpent === 0
          ? <p>No spending yet this month.</p>
          : categories.map((item) => <div key={item.category}><span>{item.category}</span><strong>{formatCad(item.amount)}</strong></div>)}
      </div>
